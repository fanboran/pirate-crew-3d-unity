...
> 关卡号段**有意不连续**：1 与 3 保留，2（碎岛雨）已删除（创始人 2026-09-22，连关卡资产一起删），
> 不重编号。
...
两关定位为教学（L1）→ 考核（L3），蓝方 luck 走 1 → 5（原版四档 1/2/5/10 的首档与第三档）——
原 1 → 2 → 5 阶梯里的第 2 档随关卡 2（碎岛雨）删除而消失。
...
> ⚠ `docs/images/level-design/difficulty-curve.png` **仍是三关版**（含已删的 L02），当前与实际两关
> 不符——需重跑 `python tools/level-design/render_curve.py` 刷新后再引用。
