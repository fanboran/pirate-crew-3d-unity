...
namespace PirateCrew.EditorTools
{
    ...
    ///   无头: -batchmode -nographics -quit -executeMethod PirateCrew.EditorTools.BattleSceneSetup.BuildAll
    ///
    /// 【对应章节】
    ...
    public static class BattleSceneSetup
    {
        ...
        public static void BuildAll()
        {
            ...
            Debug.Log("[BattleSceneSetup] Battle 战斗场景重建完成。\n"
        ...
        static void BuildBattleScene(GameObject piratePrefab)
        {
            ...
            // 像素化着色路径：相机装配（Cast / Screen / 透明件叠加）+ 内容材质转换器。
            // 必须在相机与主光都建好之后（rig 要主光的引用）。
            InstallPixelartPath(camera, sunLight);
            ...
            if (!EditorSceneManager.SaveScene(scene, BattleScenePath))
                Debug.LogError("[BattleSceneSetup] 保存场景失败: " + BattleScenePath);
        }
        ...
        /// </list>
        /// </summary>
        static void InstallPixelartPath(Camera camera, Light sunLight)
        {
            if (camera == null)
            {
                Debug.LogError("[BattleSceneSetup] 没有主相机，像素化路径未装配。");
                return;
            }
            ...
            rig.sun = sunLight;
            rig.inkColor = InkColor;

            camera.gameObject.AddComponent<PixelartContentConverter>();
        ...
        /// <summary>
        /// 3D 侧墨色：中性近黑 `#0D0D0F`。与 UI 令牌的 INK（`#120C14`，带一点紫）**故意不同**——
        /// 那个色在面板底色上稳，画在 3D 的蓝灰地面上会读成紫（创始人 2026-09-22 报的就是这个）。
        /// 值与 `PixelartCameraRig.inkColor` 的默认值同源，这里显式写一遍是为了"装配器写死、看得到"。
        /// </summary>
        static readonly Color InkColor = new Color(0.050980f, 0.050980f, 0.058824f, 1f);
        ...
        static Transform CreateWaterPlane(float worldWidth, float worldDepth, float waterWorldY)
        {
            ...
            {
                Debug.LogWarning("[BattleSceneSetup] 未找到 WaterObstacleMap.png——" +
        ...
        static void SetPrefabRefIfExists(SerializedObject so, string fieldName, string path)
        {
            ...
            if (prop == null)
            {
                Debug.LogError("[BattleSceneSetup] RuntimeSceneArt 找不到序列化字段 " + fieldName + "（字段名漂移？）");
                return;
            }
            ...
            if (prefab == null)
                Debug.LogWarning("[BattleSceneSetup] 烘焙件缺失（" + path + "）——先跑 "
            ...
            BattleTerrainView terrainView, RuntimeSceneArt runtimeSceneArt)
        {
            ...
            if (prefabComponent == null)
                Debug.LogError("[BattleSceneSetup] PirateBase 预制体缺失 PirateBase 组件，BattleController.piratePrefab 无法接线。");
            ...
            // 海面：**清掉场景里烘的旧活水材质**（`Ocean_Water.mat`，shader `PirateCrew/Ocean`）。
            // 新管线（创始人 2026-09-22「以后走新管线」）的几何只有"不透明数据 + 全屏着色"，
            // 半透明的活水面进不来；`OceanRig` 在运行期会用 `PixelartMaterialFactory` 造**不透明替身海面**
            // （平色 + 色带，与海图试点场景的替身同色）。若这里留着旧材质，运行期会优先用它
            // ——症状是海面在新管线下**整块消失**（那个 shader 没有本路径的 `PixelartOpaque` pass，
            // 不报错、就是不画）。
        ...
        static void SetRef(SerializedObject so, string name, Object value)
        {
            ...
            if (prop == null)
            {
                Debug.LogError("[BattleSceneSetup] 找不到序列化字段: " + name);
                return;
            }
        ...
        static void RunArtHook(string hookName, System.Action hook)
        {
            ...
            {
                Debug.LogWarning("[BattleSceneSetup] 美术钩子 " + hookName + " 抛出异常（已忽略，不影响场景重建）：\n" + e);
            }
        }
        ...
        static Material EnsureEnvironmentMaterial(string materialName, Color fallbackColor)
        {
            ...
            Debug.LogWarning("[BattleSceneSetup] 环境材质 " + materialName
        ...
        static Material EnsureOutlineMaterial()
        {
            ...
            if (shader == null)
            {
                ...
                Debug.LogWarning("[BattleSceneSetup] 未找到 shader " + OutlineShaderName
        ...
        static Material EnsureMaterial(string path, string name, string shaderName, Color color, string fallbackShaderName = null)
        {
            ...
            if (shader == null)
            {
                Debug.LogWarning("[BattleSceneSetup] 未找到 shader " + shaderName + "，退回 Standard。");
