...
//
// 【色空间注意】本工程 ProjectSettings m_ActiveColorSpace = 0（Gamma），
//   sRGB 十六进制直接归一化赋值即可与 GDD 色板在屏幕上一致（材质生成脚本注释里也标注了）。
//   若将来把工程切到 Linear，材质生成脚本里的调色板色必须统一改为 Color.linear，
//   否则所有材质会整体偏亮 —— 这是切换色空间最容易漏的一处。
//
// 【已知边界（提案/待定）】
...
Shader "PirateCrew/PirateSurface"
{
    Properties
    {
        ...
        // 0 正常 / 1 只 albedo / 2 噪声 / 3 法线 / 4 光滑度·金属度 / 5 湿掩码 / 6 沙纹
        // 既有 0-4 档语义不变；5、6 为本次新增（语义见片元分支注释）。
        _DebugMode              ("调试模式 0=正常 1=albedo 2=噪声 3=法线 4=光滑度金属度 5=湿掩码 6=沙纹", Range(0.0, 6.0)) = 0.0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 SurfaceFragment(VaryingsSurface IN) : SV_Target
            {
                ...
                // ---- 细节法线 ----
                float3 nrm = PirateDetailNormal(IN.positionWS, normalWS, _DetailNoiseScale, _DetailNormalStrength);
