...
namespace PirateCrew.PirateCrew.Battle
{
    ...
    public static class PlatformClusterLayout
    {
        ...
        //   2. 一张图 = 多个独立空岛，各自**不同的悬浮基准高度**（阶梯 0/6/12/18 世界单位）；
        //   3. 大块可读剪影 + 大色块（能站/会死一眼分清）；
        //   4. 禁止：贴水薄甲板 / 矩形瓦片拼盘 / 同层平铺 / 木板+草方块混搭。
        //
        // 【基准高度上限定为 18 世界单位（阶梯 0/6/12/18）的量化依据】—— 格 1→2 单位后整段 ×2
        //   · 相机可达性：BattleCameraController 全场档 = 距离 30 / 俯角 45°（FullFieldDistance/
        //     FullFieldPitchDegrees），相机高出焦点 30·sin45° ≈ 21.21 世界单位；滚轮最远 50 档
        //     则为 35.36。最高岛顶 = 18 + 5 块×0.5 = 20.5 < 21.21 → 全场档仍在画面内。
        //   · 瞄准/投掷可达性：满力投掷（twangMax=20 px/帧、抬升 0.7、g=39.06）的**竖直顶点**
        //     只有 v_v²/(2g) ≈ 4.11 世界单位（v_v = 20/0.64/√1.49·0.7 ≈ 17.92）。
        //     即单发投掷最多爬到起点上方约 4.11 单位；加上投手可站的最高局部台阶 5 块（2.5），
        //     一次跨越的**竖直档差上限 ≈ 6.6 单位**。故阶梯步长取 6（= 旧的 3 ×2）。
        //   · 一个反例记录（诚实声明）：步长 6 贴近 6.6 的上限，投手若站在最低台阶上，
        //     跨 6 档会落在射程边缘（需靠岛内高地或中继岛辅助），这是本方案的**已知手感代价**。

        /// <summary>
        /// 基准高度阶梯（世界单位）。用户裁决的 0/6/12/18 档（格 1→2 单位后 = 旧的 0/3/6/9 ×2）；
        /// 每档 = 12 块 @0.5。见类头「基准高度上限定为 18 世界单位」的量化依据。
        /// </summary>
        public static readonly float[] BaseHeightLadder = { 0f, 6f, 12f, 18f };
        ...
        /// 但两条硬约束把 N 夹在 0.573–0.643 世界单位/行（= 1.14–1.28 块/行）之间：
        ///   · 下限：level_1 各岛顶行离水线 3–10 行（7 行差）。N &lt; 0.573 单位/行时
        ///     同关最高-最低岛高差 &lt; 4 世界单位（用户裁决 ③ 的下限），关卡读不出高低结构；
        ///   · 上限：33 关里最大的岛顶行离水线 28 行（level_21 的一座高崖）。相机全场档的机位
        ///     只比聚焦点高 30·sin45° ≈ 21.21 世界单位（BattleCameraController.FullFieldDistance/Pitch），
        ///     故最高岛顶必须 ≤ 18 世界单位 → N ≤ 18 / 28 ≈ 0.643 单位/行。
        /// 取 1.25 块/行 = 0.625 单位/行（格 1→2 单位后每块 0.5）：最大 = round(28×1.25) = 35 块 = 17.5 单位 ✓；
        /// level_1 高差 = round(10×1.25) − round(3×1.25) = 12 − 4 = 8 块 = 4.0 单位 ✓（恰好达标）。
        /// 取整用 <see cref="Mathf.RoundToInt"/>（银行家舍入，跨运行时确定）。
        /// </summary>
        public const float TileBlocksPerRow = 1.25f;

        /// <summary>
        /// 单岛悬浮基准高度上限（块）：36 块 = 18 世界单位。上界推导见 <see cref="TileBlocksPerRow"/>
        ...
        static PlatformMap BuildGeneric(LevelData level)
        {
            ...
            // 用户裁决 2：给每个岛分配**错落**的悬浮基准高度（0/6/12/18 世界单位阶梯）。
        ...
        // ==================================================================
        // 用户裁决 2：错落基准高度（0/6/12/18 世界单位阶梯）
        // ==================================================================
        ...
        ///   · 出生岛贴水：anchor 出生簇（红队优先）强制落在阶梯最低档 0，另一支出生簇压到 ≤ 1 档（6），
        ///     保证开局双方站在离水面最近的岛上、画面可读（用户裁决："至少一个贴水档 0-6"）；
        ///   · 跨度 ≥ 1 档：至少两个簇拿到不同档（否则与"同层平铺"无异）；
        ///   · 阶梯连续：用到的档位必须是 0..M 的前缀（用了 18 就必须有 6/12 的中继岛）——
        ///     单发投掷的竖直顶点只有 ≈4.11 单位（见类头），跳档会让高岛变成打不到的孤岛。
        ///
        ...
        /// 用角色默认 weight=1、twangMax=20（<see cref="CrewCatalog"/>），得约 23.5 单位
        /// （= 旧口径 11.7 单位 ×2；换算回**格数**恒为 11.7 格 = 23.5 / <see cref="LevelGeometry.TileWorldSize"/>）。
        ...
        /// <summary>
        /// 两簇**最近可站格**之间的世界距离（格宽 = <see cref="LevelGeometry.TileWorldSize"/>）：
        /// 沿 X/Z 各自算边到边的距离再取欧氏合成。
        ...
        public static float MinEdgeDistanceWorld(in PlatformClusterInfo a, in PlatformClusterInfo b)
        {
            ...
            float dx = gapX >= 0 ? LevelGeometry.TileToWorld(gapX + 1f) : 0f;
            float dz = gapZ >= 0 ? LevelGeometry.TileToWorld(gapZ + 1f) : 0f;
