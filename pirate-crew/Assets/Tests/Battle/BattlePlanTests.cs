...
namespace PirateCrew.PirateCrew.Battle.Tests
{
    ...
    public class BattlePlanTests
    {
        ...
        public void Level1_FirstEntry_HasExpectedCoordinatesLuckAndType()
        {
            ...
            // 世界坐标 = GridToArena(17,10) = ((17+0.5)×2, UnitPivotHeight=0.5, (10+0.5)×2)
            // = (35, 0.5, 21)：3D 重投影后 gridY→纵深 Z、高度 Y 只有脚底贴地的 0.5 枢轴偏移
            //（格 1→2 单位后格心 = (格号+0.5)×TileWorldSize，见 LevelGeometry.cs:313）。
            Assert.AreEqual(35f, first.WorldPosition.x, 1e-5f);
            Assert.AreEqual(LevelGeometry.UnitPivotHeight, first.WorldPosition.y, 1e-5f);
            Assert.AreEqual(21f, first.WorldPosition.z, 1e-5f);
        }
        ...
        public void Level1_Plan_UsesDepthSemanticsForXZDepth()
        {
            ...
            Assert.AreEqual(100f, plan.WorldWidth, 1e-5f);
            Assert.AreEqual(34f, plan.WorldDepth, 1e-5f);
        }

        [Test]
        public void Level1_WaterWorldY_IsGlobalWaterSurfaceConstant()
        {
            // §4.4 3D 化：水面改为全局常量 WaterSurfaceY = -0.4（格 1→2 单位后由 -0.2 乘 2，
            // LevelGeometry.cs:197），不再由关卡 waterTileY(=14) 推出。
            BattlePlan plan = LevelGeometry.BuildBattlePlan(LevelCatalog.Get(1));
