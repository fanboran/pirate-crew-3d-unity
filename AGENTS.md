...
- ⚠ **视觉风格已定案，未经用户明确批准不得动**（2026-09-17 用户重申，此前已说过一次但仍被违反）：
  **角色模型不许改**——现有程序化低模船员（胶囊+三角帽，`CrewVisualPrefabBuilder` 产出）就是本游戏的
  最终风格（"这就是低多边形风格的游戏"）。Blender/外部建模管线**只允许**用在用户点名同意的资产上
  （2026-09-16 已同意的：应用图标）。任何"换个建模方式/加细节/替换网格"的想法一律先提案、等用户点头。
